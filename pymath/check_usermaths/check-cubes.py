...
def sqaure(num_to_sq):
    result = num_to_sq*num_to_sq*num_to_sq
    text="{} x {} x {} = {}"
    formatted_text=text.format(num_to_sq,num_to_sq,num_to_sq,result)
    welcome_text(formatted_text)

if __name__ == "__main__":
    clear_the_screen_first()
    welcome_text(current_data_two.square.title)
    print(current_data_two.author.msg_to_display)
    current_data_two.horizontal_line
    multiple_ques()
